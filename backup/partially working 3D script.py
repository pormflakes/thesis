...
GlassThickness = 10                              #GLASSTHICKNESS in mm   standaard maten:4,6,8,10,12
GlassHeight = 5000
...
for i in range (0,number_lines):
    ...
    mdb.models['Model-1'].Part(dimensionality=THREE_D, name=PartName, type= DEFORMABLE_BODY)
    mdb.models['Model-1'].parts[PartName].BaseSolidExtrude(depth=GlassHeight, sketch=
...
for i in range (0,number_lines):
    ...
    mdb.models['Model-1'].parts[PartName].cells.getSequenceFromMask(('[#1 ]', ), 
    ), name=SetName)
...
for i in range(0,number_lines):
    ...
    (0.0,x_coord[i], 0.0))
    #SURFACE SELECTION
    mdb.models['Model-1'].rootAssembly.Surface(name=SurfaceName, side1Faces=
    mdb.models['Model-1'].rootAssembly.instances[InstName].faces.getSequenceFromMask(
    ('[#2 ]', ), ))
    #LOAD 
    mdb.models['Model-1'].Pressure(amplitude=UNSET, createStepName='LoadStep', 
        ...
        region=mdb.models['Model-1'].rootAssembly.surfaces[SurfaceName])
